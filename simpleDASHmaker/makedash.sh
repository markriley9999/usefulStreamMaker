...
# --- Convert to 25 fps! ---
ffmpeg -i tmp/stripped.mp4  -filter_complex "[0:v]setpts=24/25*PTS[v];[0:a]atempo=25/24[a]" -map "[v]" -map "[a]" -r 25 tmp/av25fps.mp4


# --- scale and crop, aac audio ---
 ffmpeg -i tmp/av25fps.mp4 -vf "scale=-1:1080, crop=1920:1080" -c:v libx264 -r 25 -profile:v main -preset fast -c:a aac -ac 1  -movflags +faststart tmp/source.mp4
